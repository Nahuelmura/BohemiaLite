...
public JsonResult ListadoProducto(int productoID, string codigo)
{
    ...
    int totalCantidadProductos = productos.Sum(p => p.Cantidad);
     ...
     decimal totalPrecioCosto = productos.AsEnumerable().Sum(p => p.PrecioCosto * p.Cantidad);
...
return Json(new { 
    ...
    public JsonResult GuardarProducto(int productoID, string codigo, Descripcion descripcion, int cantidad, decimal precio, string observacion,decimal precioVenta)
    {
        string resultado = "";
