    ...
    $.ajax(
        {
            ...
            success: function (listadoVentas) {
                ...
                $.each(listadoVentas, function (index, ventas) {
                    ...
                    <td style="${claseCantidad}" class="ocultar-en-768px">${ventas.cantidad}</td>
...
function AgregarProductoTemporal() {
    ...
    let observacion = $("#Observacion").val() || "No definido";
    let stockDisponible = parseInt($("#stockdisponible").val());
    let forma_Pago = $("#Forma_pago option:selected").text();
    let totalConDescuento = parseFloat($("#totalConDescuento").val()) || 0; 
...
function GuardarVenta() {
    ...
    $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        data: { fecha_venta: fecha_venta, total: total, usuarioID: usuarioID, Forma_pago: formaPagoSeleccionada, },
        success: function (response) {
            if (response.success) {
